...
public class JumboPacket {
	...
	public byte[] getBytes() {
		...
		//pdu[i++]=(byte)version;
		// packet id
		ByteUtils.intToFourBytes(packet_id,pdu,i);
		i+=4;
		// packet length
		ByteUtils.intToFourBytes(packet_length,pdu,i);
		i+=4;
		// packet offset
		ByteUtils.intToFourBytes(packet_offset,pdu,i);
		i+=4;
	...
	/** Gets a JumboPacket from a array of bytes. */
	public static JumboPacket parsePacket(byte[] buf, int off, int len) {
		int k=off;
		int packet_id=(int)ByteUtils.fourBytesToInt(buf,k);
		k+=4;
		int packet_length=(int)ByteUtils.fourBytesToInt(buf,k);
		k+=4;
		int packet_offset=(int)ByteUtils.fourBytesToInt(buf,k);

		JumboPacket jumbo=new JumboPacket(buf,off+HLEN,len-HLEN,packet_id,packet_length,packet_offset);
		return jumbo;
	}
	...
	public static JumboPacket doReassemble(JumboPacket[] fragments) {
		...
		for (int i=0; i<fragments.length; i++) {
			...
			ByteUtils.copy(frag.data,frag.data_offset,data,frag.packet_offset,frag.data_length);
		}
	...
	public String getHexData() {
		return ByteUtils.asHex(data,data_offset,data_length);
	}
