...
public class UdpProvider extends Thread {
	...
	public void send(UdpPacket packet) throws IOException {
		...
		else if (DEBUG) printLog("send(): UDP has been halted: no packet sent.");
	}
		...
		try    {
			...
			while(!stop) {
				...
				if (packet.getLength()>=minimum_length) {
					...
					if (DEBUG) printLog("receive(): packet passed to "+listener);
					if (alive_time>0) expire=System.currentTimeMillis()+alive_time;
				}
			...
			error=e;
			if (DEBUG) printLog("Exception: "+e.toString());
			stop=true;
		} 
		is_running=false;
	...
	static protected void printLog(String str) {
		//local.microua.MicroUI.log.println("DBG: UdpProvider: "+str);
	}
